...
export default function MobileBottomNav({ onInsightsClick, isInsightsActive = false }: MobileBottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-[#0e0e0f] border-t border-[#2c2c2f] z-50 md:hidden">
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center min-w-[60px] pb-1.5 pt-0.5 px-1 rounded-md transition-all duration-200 ${
                active ? "text-[#d5a514] cursor-default" : "text-gray-400 hover:text-gray-200 cursor-pointer"
              }`}
            >
              <Icon className={`w-5 h-5 mb-0.5 transition-all duration-200 ${active ? "scale-105" : ""}`} />
